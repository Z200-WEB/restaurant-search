...
function BudgetPips({ code }: { code?: string }) {
    ...
    return (
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4].map(i => (
                    <span key={i} className="text-[10px]" style={{ color: i <= level ? 'var(--accent)' : 'var(--border)' }}>¥</span>span>
                  ))}
          </div>div>
        )
}
                                        ...
                                        <article
                                                ...
                                                <div className="relative h-48 overflow-hidden" style={{ background: 'var(--border)' }}>
                                                  {hasImage ? (
                                                              ...
                                                              <div className="h-full flex items-center justify-center">
                                                                            <div className="text-4xl opacity-30">🍽</div>div>
                                                              </div>div>
                                                          )}
                                                </div>div>
                                          ...
                                          {shop.genre?.name && (
                                                            <div className="absolute bottom-2.5 left-3">
                                                                        <span className="badge text-xs px-2 py-0.5">{shop.genre.name}</span>span>
                                                            </div>div>
                                                )}
                                          ...
                                          {/* Favorite button */}
                                                <button
                                                            ...
                                                            aria-label={favorited ? 'お気に入りから削除' : 'お気に入り登録'}
                                                          >
                                                          <Heart
                                                                        size={14}
