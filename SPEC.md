...
- Next.js App RouterにおけるServer ComponentとClient Componentの使い分けの最適解（特にデータフェッチのタイミング判断）
- localStorageを使ったお気に入り機能の実際のプロダクトでの設計方針（DB連携・認証との組み合わせ方）
- TypeScriptの型設計のベストプラクティス（外部APIレスポンスの型定義の扱い方）
