...
// Client Component: reacts to URL search params for pagination
export default function ResultsPage() {
  ...
  const searchDescription = [
    ...
    searchParams.get('lat') ? '現在地付近' : null,
  ]
    .filter(Boolean)
    .join(' · ')
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-semibold text-gray-900">
            {searchDescription || '検索結果'}
          </h1>
          {!state.loading && state.total > 0 && (
            <p className="text-sm text-gray-500">
              {state.total.toLocaleString()}件のお店が見つかりました
            </p>
          )}
        </div>
      ...
      {/* Results */}
      {state.loading ? (
        <LoadingGrid count={RESULTS_PER_PAGE} />
      ) : state.error ? (
        <EmptyState title="エラーが発生しました" message={state.error} />
