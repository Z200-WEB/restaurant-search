...
import { ArrowLeft, MapPin, Grid3X3, Map } from 'lucide-react'
          ...
          <div className="flex items-center gap-2 mt-0.5">
            {lat && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>半径{rangeLabels[range]}</span>
              </div>
            )}
            {!state.loading && (
              <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{state.total}件</span>
            )}
          </div>
        </div>
        ...
        <div className="mb-6 rounded-2xl overflow-hidden animate-fade-in" style={{ border: '1px solid var(--border)', height: '380px' }}>
          {mapUrl ? (
            ...
            <div className="h-full flex items-center justify-center" style={{ background: 'var(--bg-card)' }}>
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>位置情報が必要です</p>
            </div>
          )}
        </div>
      )}
