...
function ResultsContent() {
  ...
  const [state, setState] = useState<SearchState>({ shops: [], total: 0, loading: true, error: null })
  ...
  const fetchShops = useCallback(async () => {
    ...
    try {
      ...
      setState({ shops: data.shops || [], total: data.total || 0, loading: false, error: null })
    } catch (err) {
      setState(prev => ({ ...prev, loading: false, error: err instanceof Error ? err.message : '検索に失敗しました' }))
    }
  ...
  const rangeLabels: Record<string, string> = { '1':'300m', '2':'500m', '3':'1km', '4':'2km', '5':'3km' }
      ...
      <div className="flex items-center gap-3 mb-6 animate-fade-in">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            {lat && <div className="flex items-center gap-1"><MapPin size={11} style={{ color: 'var(--accent)' }} /><span className="text-xs" style={{ color: 'var(--text-secondary)' }}>半径{rangeLabels[range]}</span></div>}
            {!state.loading && state.total > 0 && <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{state.total}件</span>}
          </div>
        </div>
      </div>
...
export default function ResultsPage() {
  return (
    <Suspense fallback={<div className="max-w-5xl mx-auto px-4 py-6"><LoadingGrid count={9} /></div>}>
      <ResultsContent />
    </Suspense>
  )
}
